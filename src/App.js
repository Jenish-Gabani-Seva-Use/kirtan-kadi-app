...
function App() {
  ...
  const processText = (text) => {
    ...
    // Clear ALL existing shortcuts and reset state
    setSelectedLines([]); // This clears the right panel completely
    setLinesToDelete([]); // Clear any pending deletions
    ...
    if (lines.length > 0) {
      ...
      setCurrentDisplayedText(lines[0]); // Show first line of new text
    } else {
      setSelectedLineIndex(-1);
      setCurrentDisplayedText(''); // Clear display if empty
    }
  };
...
<InputModal
  ...
  currentText={currentDisplayedText} // This is what you need to add
/>
